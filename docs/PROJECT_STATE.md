...
## 33. 2026-07-06 TASK-RR-82 第二十八轮项目用户视角评测与 CLI / 只读入口语义对齐

状态：done

本轮评测报告：

- `docs/reports/PROJECT_USER_REVIEW_2026_07_06_ROUND28.md`

本轮发现并修复：

- README 只读打开方式只写“无写 API”，已明确静态打开不具备写 API、练习脚本运行、动作日志或浏览器终端能力。
- `docs/WORKSPACE.md` 对直接打开 `progress.html` 的限制说明过轻，已同步为只读布局 / 进度检查并列出缺失能力。
- README 与 `CONVERSION_PROTOCOL.md` 仍把 CLI 用途写成“补标记 / 标记完成”，已改为“补录完成 / 记录完成（CLI 补录）”。
- `rounds/round_03/README.md` 仍说脚本不会替用户“打卡”，已改为不会替用户记录完成。
- Round 00 综合验收脚本结束提示“Round 00 全部完成”容易误导，已改为“Round 00 综合验收脚本已完成”。
...
## 34. 2026-07-06 TASK-RR-83 第二十九轮项目用户视角评测与转换协议版本 / 边界对齐

状态：done

本轮评测报告：

- `docs/reports/PROJECT_USER_REVIEW_2026_07_06_ROUND29.md`

本轮发现并修复：

- `CONVERSION_PROTOCOL.md` 标题和版本表仍停在 v2.0，已更新到 v2.1。
- 转换协议开头的进度系统范围仍是旧四件套，已补当前 Web UI、本地 API、生成数据、动作日志和任务反馈组合。
- 转换协议 Round 00 状态表备注写“已完成最小闭环”，已改为材料已展开并接入 Web UI，真实学习完成状态以 `progress.json` 为准。
- `docs/WORKSPACE.md` 顶部静态打开说明仍过于含糊，已补只读布局 / 进度检查和缺失能力。
- Round 03 自动记录边界与 README 当前下一步指引仍有局部歧义，已分别明确只自动记录练习任务，并把下一步改为继续做 Web UI / 文档用户视角评测。
...
## 35. 2026-07-06 TASK-RR-84 第三十轮项目用户视角评测与规则版本 / 下一步对齐

状态：done

本轮评测报告：

- `docs/reports/PROJECT_USER_REVIEW_2026_07_06_ROUND30.md`

本轮发现并修复：

- `CONVERSION_PROTOCOL.md` v2.1 的生效日期仍写 2026-05-12，已改为 2026-07-06 并保留 v2.0 生效日期说明。
- 转换协议“重大变更”仍只描述 v2.0，已补 v2.1 对当前 Web UI / 本地 API / 生成数据 / 动作日志 / 任务反馈维护边界的同步。
- README 当前下一步仍说把软考模块骨架变成可检查产物清单，已改为选择 1-2 个软考模块开始写概念地图。
- `docs/PROGRESS_RULES.md` 版本仍是 v1.0，已升为 v1.1。
- 进度规则开头仍把 Web UI 简化为 `progress.html` 展示，且未说明 `progress.json` 顶层结构，已补当前文件分工与 `version / lanes / tasks` 说明。
...
## 36. 2026-07-06 TASK-RR-85 第三十一轮项目用户视角评测与移动端教程 / 终端联动优化

状态：done

本轮评测报告：

- `docs/reports/PROJECT_USER_REVIEW_2026_07_06_ROUND31.md`

本轮发现并修复：

- API 成功横幅在正常状态下仍占用首屏空间，已改为成功时隐藏，仅保留需要处理的警告。
- 移动端点击“读教程”后没有直接滚到教程面板，已改为移动端优先滚到教程面板。
- 移动端点击“终端练习”后没有直接滚到终端面板，已改为移动端优先滚到终端面板并聚焦输入框。
- 教程面板和终端面板之间缺少互跳入口，已新增“去终端”和“回到教程”。
- 终端快捷命令与重置行为不够贴合当前任务，已增加 week 上下文 `self_check` 命令，并让重置直接回到 `~/cli-lab`。

验证摘要：

- `python3 scripts/agent_gate.py --verify` 已通过。
- 浏览器默认桌面、Round 02 桌面和 Round 02 移动端已确认页面可加载、无横向溢出；移动端 Round 02 已确认成功横幅隐藏、快速切换条出现、教程/终端滚动联动、快捷命令和终端重置行为正常。

## 37. 2026-07-06 TASK-RR-86 第三十二轮项目用户视角评测与非终端任务入口 / 记录弹窗优化

状态：done

本轮评测报告：

- `docs/reports/PROJECT_USER_REVIEW_2026_07_06_ROUND32.md`

本轮发现并修复：

- 默认软考阅读任务不需要终端，但移动端快速切换条仍显示“终端”，已在 no-terminal 状态下隐藏该入口。
- 非终端任务教程面板仍显示“去终端”，已在 no-terminal 状态下隐藏。
- 移动端仍显示“单独打开教程”，容易回到遮挡工作区的弹窗阅读模式，已在移动端隐藏。
- 记录弹窗的可写示例只藏在 placeholder 里，已增加可见“记录参考 / 证据示例”。
- 动作记录里 `run_exercise` 显示为“运行练习”，已统一为“运行脚本”。

验证摘要：

- `python3 scripts/agent_gate.py --verify` 已通过。
- 浏览器默认桌面、默认移动端和 Round 02 移动端已确认非终端 / 终端任务入口区分正确，记录弹窗显示记录参考，页面无横向溢出。

## 38. 2026-07-06 TASK-RR-87 第三十三轮项目用户视角评测与 lane 级反馈 / 记录示例优化

状态：done

本轮评测报告：

- `docs/reports/PROJECT_USER_REVIEW_2026_07_06_ROUND33.md`

本轮发现并修复：

- 任务反馈生成器对所有未开始任务使用同一句通用建议，已改为按 lane 区分软考、数学二、408 和工程实操。
- 默认软考任务提示仍像工程练习提示，已通过重新生成反馈 JSON 改为易混点 / 概念地图导向。
- 记录弹窗可见示例出现“证据示例：例如：...”重复，已去掉可见示例中的 `例如：` 前缀。
- 记录弹窗证据路径缺少强制换行，已增加 `overflow-wrap: anywhere`。
- 桌面端教程弹窗按钮“单独打开教程”未说明弹窗性质，已改为“弹窗查看”。

验证摘要：

- `python3 scripts/agent_gate.py --verify` 已通过。
- 浏览器默认桌面、默认移动端和 Round 02 移动端已确认 lane 级建议、记录示例、弹窗按钮和横向溢出状态正常。
