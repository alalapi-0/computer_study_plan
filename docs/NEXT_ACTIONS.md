...
- 风险：让示例数据被误认为真实进度。
- 是否需要用户介入：否。
- 实际产物：`records/weekly_reviews/_example.md`（明确标注示例，非真实数据）。
...
- 是否需要用户介入：否。
- 实际产物：`plans/math2/limits.md`、`plans/math2/la_matrix.md`（官方大纲提示 + 小节骨架 + 各 1 条启动级易错点，无题库缓存）。
...
- 前置：用户提供目标院校 + 官网招生目录链接，或允许 Codex 使用 full_network 权限访问目标官网。
- 风险：基于经验贴 / 过期数据填写。
- 是否需要用户介入：**是**。
...
1. **TASK-RR-03**：Round 02 实操目录展开（可选，无需用户输入）。
2. **TASK-RR-05 / 07**：院校信息 + 考试日期（需用户）。
