...
本轮可以通过当前本地 Web UI 完成：

1. 启动本地服务：`python3 scripts/progress_server.py --host 127.0.0.1 --port 8777`。
2. 打开 `http://127.0.0.1:8777/progress.html?round=round_20`。
3. 按顺序点击每周“读教程”任务，在页面中间读完 notes。
4. 点击“运行脚本”任务，让系统在 `~/cli-lab/round20` 下生成 PyTorch 风格示例、标准库 smoke check 和静态检查报告。
5. 点击“终端练习”任务，在浏览器映射终端手写最小自测脚本，确认 tensor 形状、训练循环四步和 checkpoint 语义。
6. 最后完成小抄与验收任务，把“Tensor -> Dataset/DataLoader -> nn.Module -> train/eval -> save/load”讲清楚，并用“记录并完成”保存本次记录。
