...
2. 浏览器打开 `http://127.0.0.1:8765/progress.html?round=round_07`。
3. 在 Round 07 任务列表里按顺序完成：
   - 阅读任务：点“阅读”，直接在页面中读 `notes.md`、脚本和 `round_07.md`。
   - 自动练习：点“运行”，脚本会在 `~/cli-lab/round7/*_auto` 生成演示数据、输出结果、写日志并自动记录对应练习完成。
   - 自测任务：点“终端”，在浏览器映射终端中自己创建文件、运行脚本；确认理解后再手动点“记录 / 完成”。
   - 小抄与验收：读 `final/ai_prep_tool_cheatsheet.md`，能解释后再手动完成记录。
...
- `r07-w*-self`、`r07-fin-sheet`、`r07-fin-acc1` 必须由用户通过 Web UI 阅读、手写、自测和手动记录。
