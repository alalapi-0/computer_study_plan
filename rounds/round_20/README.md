...
这个目录让 Round 20 可以通过 Web UI 完成：页面内阅读 notes，点击“运行脚本”生成可检查练习产物，再用浏览器内终端完成手写自测。
...
1. 打开 `http://127.0.0.1:8777/progress.html?round=round_20`。
2. 每周先点“读教程”，在弹窗中读完概念和自测命令。
3. 点“运行脚本”生成自动练习产物。所有产物写入 `~/cli-lab/round20`。
4. 点“终端练习”完成手写自测，再点击“记录并完成”保存自测、小抄和最终验收任务。
...
- 脚本只自动记录对应“练习”任务；阅读、自测、小抄和验收仍由用户自己完成后点击“记录并完成”保存记录。
