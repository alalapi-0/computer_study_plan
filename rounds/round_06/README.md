...
1. 在仓库根启动服务：`python3 scripts/progress_server.py`。
2. 浏览器打开：`http://127.0.0.1:8777/progress.html?round=round_06`。
3. 先点每周“读教程”任务的“读教程”，在页面内读 notes。
4. 再点“练习”任务的“运行脚本”，脚本会在 `~/cli-lab/round6` 下生成安全练习产物，并只标记脚本实际完成的练习任务。
5. 自测任务点“终端练习”，浏览器终端会绑定到 `~/cli-lab/round6`，命令历史会带上 `task_id`。
6. 自测、小抄和最终验收都需要用户自己理解后，点击“记录并完成”。
