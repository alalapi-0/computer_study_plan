...
- 工作日优先打开 Web UI 的「学习工作区」：左侧确认当前任务，中间读教程，按任务做练习或整理要点；工程任务出现「终端练习」时再绑定浏览器终端。完成一个最小动作后，写一条记录并点击「记录并完成」。
...
- **方式一（推荐 · 网页打卡）**：
  ...
  open http://127.0.0.1:8777/progress.html
  ```
  看板首屏是「学习工作区」：当前任务和教程阅读优先展示；工程任务会额外显示浏览器终端。点击「读教程」会在页面中间打开资料；点击「终端练习」会把终端绑定到对应 `~/cli-lab/roundN` 沙盒。完成后需写一条记录，再「记录并完成 / 撤销」任务。存档 / 读档放在折叠的管理区中，读档前会自动创建恢复点。
