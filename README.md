...
- **方式二（本地 HTTP，当前推荐）**：
  ...
  ```
- **方式三（规划中 · 网页内完成全部学习操作）**：见 [`docs/PROGRESS_WEB_LEARNING_ROADMAP.md`](docs/PROGRESS_WEB_LEARNING_ROADMAP.md)——目标是在同一 URL 阅读 notes、做练习、打卡、写错题与复盘（`learn_server` + PW-0 ~ PW-6，**尚未实现**）。
