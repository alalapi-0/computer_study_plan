...
`commands.jsonl` 由 Web UI 的“终端练习”自动追加写入，用来保存浏览器沙盒终端中的命令历史。

该文件是本地操作历史，默认被 `.gitignore` 排除；需要长期沉淀的结论应写入学习记录或周复盘。
...
- 从任务行点击“终端练习”后执行的命令会带上 `task_id`，方便回看命令属于哪个学习任务。
- 这些记录只说明“执行过命令”，不等同于任务完成；任务完成仍以 Web UI 的“记录并完成 / 记录回看”或 `mark_done.sh` 为准。
